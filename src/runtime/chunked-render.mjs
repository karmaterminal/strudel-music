...
const sampleFileNames = new Map();  // "sound:n" → original filename (for root note detection)
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (buf) {
          ...
          sampleFileNames.set(`${dir}:${i}`, files[i].replace(/\.\w+$/, ''));
          if (i === 0) {
            sampleBufferData.set(dir, buf);
            sampleFileNames.set(dir, files[i].replace(/\.\w+$/, ''));
          }
          sampleCount++;
        }
      }
  ...
  for (const hap of haps) {
    ...
    if (sampleBuf) {
      ...
      if (v.note) {
        ...
        // Use actual filename for root detection (e.g. "bass_Cs1" → C#1/MIDI 25)
        const fileName = sampleFileNames.get(sampleKey) || sampleFileNames.get(sound) || sound;
        const rootMidi = detectRootNote(fileName);
